            ...
            <div className="w-full space-y-3 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-2xl">
                    {visual?.emoji || <Bot className="h-5 w-5" />}
                  </div>
                  <div className="min-w-0">
                    <p className="truncate font-medium text-white">{agent.name}</p>
                    <p className="line-clamp-2 text-sm leading-5 text-slate-400">{agent.role}</p>
                  </div>
                </div>

                <Badge variant="outline" className="shrink-0 border-white/10 text-slate-300">
                  {agent.expertise.length} frentes
                </Badge>
              </div>
